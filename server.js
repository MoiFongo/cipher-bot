...
async function initDB() {
  await pool.query(`CREATE TABLE IF NOT EXISTS trades (
    id SERIAL PRIMARY KEY, pair TEXT, side TEXT DEFAULT 'sell',
    entry FLOAT, exit FLOAT, profit FLOAT, confidence FLOAT,
    features JSONB, timestamp TIMESTAMP DEFAULT CURRENT_TIMESTAMP
  );`);
}
...
app.get("/api/status", auth, async (req, res) => {
  ...
  const market = {};
  for (const pair of PAIRS) {
    market[pair] = { price: history[pair]?.prices?.at(-1) ?? null, prices: history[pair]?.prices?.slice(-40) ?? [] };
  }
