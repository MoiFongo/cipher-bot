...
async function getMarket() {
  ...
  for (const key in data.result) {
    out[PAIRS[i]] = {
      ...
      change24h:
        (parseFloat(data.result[key].p[1]) > 0
          ? (parseFloat(data.result[key].c[0]) - parseFloat(data.result[key].p[1])) /
            parseFloat(data.result[key].p[1])
          : 0)
    };
    i++;
  }
...
async function predict(f) {
  ...
  for (const row of res.rows) {
    ...
    const sim =
      1 -
      Math.abs((f.shortTrend || 0) - (tf.shortTrend || 0)) -
      Math.abs((f.longTrend || 0) - (tf.longTrend || 0));
...
<style>
:root{
  --bg:#0b0d12; --card:#141923; --card2:#0f131b; --text:#f3f6fb; --muted:#9aa4b2;
  --green:#17c964; --red:#f31260; --blue:#3b82f6; --orange:#f59e0b; --border:#242b38;
}
...
<div class="wrap">
  ...
  <div class="grid">
    <div class="card">
      ...
      <div class="small">Drawdown: <span id="drawdown">--</span></div>
    </div>
    <div class="card">
      <div class="kpi">Performance</div>
      <div class="val" id="pnl">--</div>
      <div class="small">Win Rate: <span id="winRate">--</span></div>
    </div>
    <div class="card">
      <div class="kpi">Open Positions</div>
      <div class="val" id="openCount">0</div>
      <div class="small">Losing Streak: <span id="streak">0</span></div>
    </div>
    <div class="card">
      <div class="kpi">Controls</div>
      <div class="row" style="margin-top:10px">
        <button class="btn btn-primary" id="toggleBtn">Toggle Bot</button>
        <button class="btn btn-danger" id="sellBtn">Sell All</button>
        <button class="btn btn-ghost" id="balanceBtn">Balance</button>
      </div>
    </div>
  </div>
  ...
  <div class="section grid">
    <div class="card">
      <h3>📈 Equity Curve</h3>
      <div class="canvasBox"><canvas id="equityChart"></canvas></div>
    </div>
    <div class="card">
      <h3>₿ BTC / ⟠ ETH / ◎ SOL</h3>
      <div class="canvasBox"><canvas id="priceChart"></canvas></div>
    </div>
  </div>
  ...
  <div class="section grid">
    ...
    <div class="card">
      <h3>🪟 Live Trading Window</h3>
      <div id="trades" class="list scroll"></div>
    </div>
  </div>
  ...
  <div class="section">
    <div class="card">
      <h3>💼 Balance</h3>
      <pre id="balanceView">Tap Balance to load account balances.</pre>
    </div>
  </div>
</div>
...
function drawLine(canvasId, series, stroke){
...
function drawMulti(canvasId, datasets){
  const c = document.getElementById(canvasId);
  const ctx = c.getContext("2d");
  const dpr = window.devicePixelRatio || 1;
  const w = c.clientWidth || 320;
  ...
  const colors = ["#f59e0b","#3b82f6","#17c964"];
  const all = datasets.flatMap(d => d.series || []);
  if (all.length < 2) {
    ctx.fillStyle = "#9aa4b2";
    ctx.font = "14px sans-serif";
    ctx.fillText("Waiting for data...", 14, 24);
...
async function refreshStatus(){
  ...
  live.textContent = data.tradingEnabled ? "LIVE" : "STOPPED";
  live.className = "badge " + (data.tradingEnabled ? "live" : "off");

  const posWrap = document.getElementById("positions");
  posWrap.innerHTML = data.positions.length
    ? data.positions.map(p => \`
      <div class="item">
        <div><strong>\${p.pair}</strong></div>
        <div class="muted">Entry: \${fmtNum(p.entry)} | Peak: \${fmtNum(p.peak)} | Conf: \${fmtNum((p.confidence||0)*100)}%</div>
      </div>
    \`).join("")
    : '<div class="item muted">No open positions</div>';

  const tradeWrap = document.getElementById("trades");
  tradeWrap.innerHTML = data.trades.length
    ? data.trades.map(t => \`
      <div class="item">
        <div><strong>\${t.pair}</strong> <span class="\${Number(t.profit)>=0?'pos':'neg'}">\${fmtPct(Number(t.profit)*100)}</span></div>
        <div class="muted">Entry: \${fmtNum(t.entry)} | Exit: \${fmtNum(t.exit)} | Conf: \${fmtNum((t.confidence||0)*100)}%</div>
      </div>
    \`).join("")
    : '<div class="item muted">No trades yet</div>';

  const eqSeries = data.trades.length
    ? data.trades.slice().reverse().reduce((arr, t) => {
        const prev = arr[arr.length - 1];
        arr.push(prev * (1 + Number(t.profit || 0)));
        return arr;
      }, [100])
    : [100];
  drawLine("equityChart", eqSeries, "#3b82f6");

  drawMulti("priceChart", [
    { name: "BTC", series: data.market["BTC/USD"]?.prices || [] },
    { name: "ETH", series: data.market["ETH/USD"]?.prices || [] },
    { name: "SOL", series: data.market["SOL/USD"]?.prices || [] }
  ]);
}
...
document.getElementById("sellBtn").onclick = async () => {
  const r = await api("/sellall", { method: "POST" });
  alert(await r.text());
  refreshStatus();
};
...
document.getElementById("balanceBtn").onclick = async () => {
  ...
  document.getElementById("balanceView").textContent = JSON.stringify(data, null, 2);
};
